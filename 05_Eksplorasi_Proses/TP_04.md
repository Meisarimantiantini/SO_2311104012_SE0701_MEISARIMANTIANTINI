# <h1 align="center">Laporan Praktikum Modul 03 <br> Eksplorasi Proses Xinu</h1>
...
1. a. 50 proses
   b. 16 karakter
   c. tergantung input saat create(), tapi default umum = 20
   d. 6 state
      PR_FREE → slot kosong, PR_CURR → sedang berjalan,PR_READY → siap dijalankan,PR_RECV → menunggu pesan,PR_SLEEP → sedang tidur (delay),PR_SUSP → suspend (dihentikan sementara).
2.  ![Import VM](image/no2codingan.png)
    ![Import VM](image/no2hasil.png)
3.  ![Import VM](image/no3.png)
4.  ![Import VM](image/no4.png)
